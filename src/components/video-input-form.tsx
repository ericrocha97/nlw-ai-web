...
import { FileVideo, Upload, Check, Trash } from "lucide-react";
...
export function VideoInputForm(props: VideoInputFormProps) {
  ...
  async function convertVideoToAudio(video: File) {
    ...
    ffmpeg.on("progress", (progress) => {
      ...
      console.log(
        `progress `,
        (Math.round(progress.progress * 100) * 60) / 100
      );
  ...
  const progressBarValue = Math.min(
    status === "converting" ? uploadVideoProgress : progressBarValues[status],
    100
  );
  ...
  return (
    <form onSubmit={handleUploadVideo} className="space-y-2">
      {videoFile && (
        <div className="relative border flex rounded-md aspect-video cursor-pointer border-dashed text-sm flex-col gap-2 items-center justify-center text-muted-foreground hover:bg-primary/5">
          <video
            src={previewURL}
            autoPlay={true}
            loop={true}
            muted={true}
            controls={false}
            className="pointer-events-none inset-0 absolute rounded-md"
          />
          <Button
            variant={"destructive"}
      ...
      <input
        ...
        id="video"
        className="sr-only"
        onChangeCapture={handleFileSelected}
      />
      ...
      status !== "success" ? (
        <div className="relative w-full h-9 rounded-md flex items-center justify-center">
          <Progress
            ...
            className="absolute h-9 rounded-md"
          />
          <span className="absolute text-sm">
            {progressBarMessages[status]}
          </span>
        </div>
      ) : (
        <Button
